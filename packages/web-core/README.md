...
[`@microsurveys/react`](https://www.npmjs.com/package/@microsurveys/react)) to render. No UI, no
...
```bash
npm install @microsurveys/web-core
```

```ts
import { createClient } from "@microsurveys/web-core";
...
Most apps use a framework adapter (`@microsurveys/react`) rather than this directly. See the
